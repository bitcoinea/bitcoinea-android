...
import static org.bitcoinea.core.Preconditions.checkState;
...
public class ShapeShift extends Connection {
    ...
    public ShapeShiftEmail requestEmailReceipt(String email, ShapeShiftTxStatus txStatus)
        ...
        try {
            ...
            Preconditions.checkState(txStatus.status == ShapeShiftTxStatus.Status.COMPLETE,
                    "Transaction not complete");
            requestJson.put("txid", Preconditions.checkNotNull(txStatus.transactionId, "Null transaction id"));
        } catch (Exception e) {
            throw new ShapeShiftException("Could not create a JSON request", e);
        }
