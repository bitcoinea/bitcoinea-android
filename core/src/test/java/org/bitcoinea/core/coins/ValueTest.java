...
public class ValueTest {
    ...
    public void runTestParseValue(ValueType type) {
        ...
        Assert.assertEquals(cent, Value.parse(type, "0.01"));
        Assert.assertEquals(cent, Value.parse(type, "1E-2"));
        assertEquals(type.oneCoin().add(cent), Value.parse(type, "1.01"));
        assertEquals(type.oneCoin().negate(), Value.parse(type, "-1"));
        try {
            Value.parse(type, "2E-20");
    ...
    public void runTestParseValue2(ValueType type) {
        ...
        Assert.assertEquals(cent, Value.parse(type, bigDecimalCent));
        assertEquals(type.oneCoin().add(cent), Value.parse(type, BigDecimal.ONE.add(bigDecimalCent)));
        assertEquals(type.oneCoin().negate(), Value.parse(type, BigDecimal.ONE.negate()));
        try {
            Value.parse(type, new BigDecimal("2E-20"));
            org.junit.Assert.fail("should not have accepted fractional satoshis");
        } catch (ArithmeticException e) {}
    }
    ...
    public void runTestValueOf(ValueType type) {
        try {
            Value.valueOf(type, 1, -1);
            fail();
        } catch (IllegalArgumentException e) {}
        try {
            Value.valueOf(type, -1, 0);
            fail();
        } catch (IllegalArgumentException e) {}
    }
    ...
    public void runTestOperators(ValueType type) {
        assertTrue(Value.valueOf(type, 1).isPositive());
        assertFalse(Value.valueOf(type, 1).isNegative());
        assertFalse(Value.valueOf(type, 1).isZero());
        assertFalse(Value.valueOf(type, -1).isPositive());
        assertTrue(Value.valueOf(type, -1).isNegative());
        assertFalse(Value.valueOf(type, -1).isZero());
        assertFalse(Value.valueOf(type, 0).isPositive());
        assertFalse(Value.valueOf(type, 0).isNegative());
        assertTrue(Value.valueOf(type, 0).isZero());

        assertTrue(Value.valueOf(type, 2).isGreaterThan(Value.valueOf(type, 1)));
        assertFalse(Value.valueOf(type, 2).isGreaterThan(Value.valueOf(type, 2)));
        assertFalse(Value.valueOf(type, 1).isGreaterThan(Value.valueOf(type, 2)));
        assertTrue(Value.valueOf(type, 1).isLessThan(Value.valueOf(type, 2)));
        assertFalse(Value.valueOf(type, 2).isLessThan(Value.valueOf(type, 2)));
        assertFalse(Value.valueOf(type, 2).isLessThan(Value.valueOf(type, 1)));
    }
