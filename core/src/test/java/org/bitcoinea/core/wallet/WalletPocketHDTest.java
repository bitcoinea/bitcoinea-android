package org.bitcoinea.core.wallet;
...
import org.bitcoinea.core.network.interfaces.BlockchainConnection;
import org.bitcoinea.core.network.interfaces.TransactionEventListener;
import org.bitcoinea.core.protos.Protos;
import org.bitcoinea.core.wallet.exceptions.AddressMalformedException;
import org.bitcoinea.core.wallet.exceptions.Bip44KeyLookAheadExceededException;
import org.bitcoinea.core.wallet.exceptions.KeyIsEncryptedException;
import org.bitcoinea.core.wallet.exceptions.MissingPrivateKeyException;
...
public class WalletPocketHDTest {
    ...
    public void serializeUnencryptedEmpty() throws Exception {
        ...
        Assert.assertEquals(walletPocketProto.toString(), newPocket.toProtobuf().toString());
    ...
    public void serializeEncryptedEmpty() throws Exception {
        ...
        Assert.assertNotEquals(pocket.toProtobuf().toString(), newPocket.toProtobuf().toString());

        newPocket.decrypt(aesKey);

        Assert.assertEquals(pocket.toProtobuf().toString(), newPocket.toProtobuf().toString());
    }
...
//    }
    ...
    private HashMap<Address, ArrayList<ServerClient.HistoryTx>> getDummyHistoryTXs() throws AddressFormatException, JSONException {
        HashMap<Address, ArrayList<ServerClient.HistoryTx>> htxs = new HashMap<Address, ArrayList<ServerClient.HistoryTx>>(40);

        for (int i = 0; i < statuses.length; i++) {
            List<ServerClient.HistoryTx> utxList = (List<ServerClient.HistoryTx>) ServerClient.HistoryTx.fromArray(new JSONArray(unspent[i]));
...
//        final HashMap<Address, ArrayList<UnspentTx>> utxs;
        final HashMap<Address, ArrayList<ServerClient.HistoryTx>> historyTxs;
...
//        }
        ...
        public void getHistoryTx(AddressStatus status, TransactionEventListener listener) {
            List<ServerClient.HistoryTx> htx = historyTxs.get(status.getAddress());
            if (status == null) {
